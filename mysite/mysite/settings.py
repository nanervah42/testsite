...
STATIC_URL = '/static/'
STATIC_ROOT = BASE_DIR / 'static'   # в эту папку будет собираться вся статика после команды collectstatic (делается при деплое)
STATICFILES_DIRS = [
    BASE_DIR / 'mysite/static'
]   # также с этих каталогов будет создана вся статика и помещена в STATIC_ROOT
